...
private:
  ...
  void Serve() {
    for (const auto &response : responses_) {
      ...
      [[maybe_unused]] auto _r = ::read(fd, request.data(), request.size());
...
TEST(EndToEndTest, StreamsLargeResponseUnderSlowClient) {
  ...
  std::thread backend([&] {
    ...
    [[maybe_unused]] auto _w = ::write(signal[1], "d", 1);
  });
  ...
  RunGateway(config::Config{{route}}, [&](gateway::Gateway &, std::uint16_t port, int wake) {
    ...
    if (!WriteAll(client.Fd(), request, TestDeadline(), error)) {
      [[maybe_unused]] auto _w = ::write(wake, "q", 1);
      return;
    }
...
TEST(EndToEndTest, ClientDisconnectStopsUpstreamRead) {
  ...
  std::thread backend([&] {
    ...
    (void)::close(fd);
    [[maybe_unused]] auto _w = ::write(signal[1], "c", 1);
  });
